...
func TestOutDirsSetOutsOnTarget(t *testing.T) {
	...
	fooContent := []byte("this is the content of foo")
	barContent := []byte("this is the content of bar")

	fooDigest := digest.NewFromBlob(fooContent)
	barDigest := digest.NewFromBlob(barContent)
	...
	server.blobs[fooDigest.Hash] = fooContent
	server.blobs[barDigest.Hash] = barContent
